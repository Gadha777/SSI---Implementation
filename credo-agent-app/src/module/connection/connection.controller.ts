...
export class ConnectionController {
  ...
  @ApiOperation({ summary: 'Get the connectionId.' })
  async getAcmeConnectionId(@Query('id') id: string) {
    const result = await this.connectionservice.getConnectionIdAcme(id);
    return result;
  }
  @Get('Bob-connectionId')
  @ApiOperation({ summary: 'Get the connectionId.' })
  async getBobConnectionId(@Query('id') id: string) {
    const result = await this.connectionservice.getConnectionIdBob(id);
    return result;
  }
}
